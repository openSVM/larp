...
This junior engineer is forgetful so sometimes they make the changes. It might be the case that the changes which are done might delete some part of the code, so keep that in mind when selecting the 3 options below to perform on the changes, if the change made by the user junior engineer is an indication of leaving the code as it, for example: "// ...", "// ... existing code", "// rest of the code ..." just "accept current change"
...
- The git diff presented to you might result in incomplete code when you select Accept Current Change, that is fine because the rest of the code is implemented below
- If you select Accept Incoming Change, and the code will be incomplete, that is also fine because the code after this is complete
- Even if the code is incomplete, "// rest of the code ..." implies that no matter what option you chose the code will be complete and correct
- If you select Accept Current Change, then you are only keeping the code between "<<<<<<<" and "=======" and only that section
- if you select Accept Incoming Change, then you are only keeping the code between "=======" and ">>>>>>>"
...
// rest of the code ..."#
    ...
    let user_action = format!(
        r#"Chose one of 'Accept Current Change', 'Accept Incoming Change', 'Accept Both Changes'. First explain your reasoning and then put your answer in <answer>{{your_answer}}</answer>"#
    );
    vec![prompt, user_action]
}
